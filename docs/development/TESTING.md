...
- 领域模型校验：任务名称、启用步骤、键盘连按次数、组合键和文本输入等基础规则。
- 执行引擎状态流转：启动、暂停、继续、停止、停止日志写入和键盘动作分发。
- SQLite 仓储：任务、鼠标步骤、按键步骤、组合键步骤和文本输入步骤的保存、读取和删除。
