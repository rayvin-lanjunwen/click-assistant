...
**当前版本：v0.14.0** | Android 端：v0.5.0 | Windows 端：v0.14.0
...
**近期重要变更（v0.12.0）**：移除了独立等待步骤，步骤间等待统一由下一步骤的前等待控制；新增可视化步骤标记覆盖层，支持在目标界面显示序号圆点并拖动调整坐标；新增版本化数据库迁移框架、Android CI 与单元测试；Android 应用已缩小辅助功能事件范围并禁止数据备份；Android 端 UI 已按现代 iOS 卡片风格重设计，包含底部导航、权限引导页和带搜索筛选的任务库。
