...
Click Assistant 是一个本地点击与键盘输入辅助工具，目标是帮助用户把重复性鼠标点击、键盘输入和文本输入操作配置成可复用任务，并支持启动、暂停、停止和查看执行日志。

当前项目已进入编码实现阶段，首版 C# + WPF MVP 已完成基础工程骨架、任务管理、步骤配置、坐标捕获、SQLite 存储、鼠标点击执行、键盘连按执行、组合键执行、文本输入、全局停止快捷键、执行前安全确认、快捷键自定义、核心自动化测试和基础 CI。移动端已进入 Android-first 可行性原型阶段，已补充需求草案、低保真原型和 `mobile/android/` 原型工程。
...
键盘步骤支持常用单键连按，例如 `A-Z`、`0-9`、`F1-F24`、`Enter`（回车）、`Space`（空格）、`Tab`、`Esc`、方向键、`Delete`（删除）和 `Backspace`（退格）。组合键步骤支持类似 `Ctrl+C`、`Ctrl+V`、`Ctrl+Shift+Esc` 和 `Alt+Tab` 的格式。文本输入步骤支持输入一段文本，并可配置字符间隔。
...
下一阶段建议双线推进：一条线继续完善 Windows 安装包、GitHub Releases、异常提示、快捷键冲突降级测试和更多 UI 交互测试；另一条线安装 Android SDK、Gradle、ADB 或 Android Studio，对 `mobile/android/` 原型生成 APK 并在真机上验证辅助功能授权、固定坐标点击、停止入口和本地任务保存。
