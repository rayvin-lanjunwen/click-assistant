...
- 2026-07-12 06:00 Phase E 逐元素精修：Android 端 19 处 — 渐变标题（首页/编辑器/个人中心，LinearGradient #3978F6→#7067D8 + 阴影）、添加步骤按钮改为虚线边框玻璃卡片、步骤编号改为半透白底+颜色边框玻璃圆形、任务库空状态改为玻璃卡片（📋图标+创建提示）、筛选标签改为玻璃胶囊、返回按钮改为玻璃胶囊、输入框改为玻璃内嵌（圆角内陷背景）、任务卡片图标升级为玻璃圆形（白底+颜色边框+elevation）、快捷执行按钮改为玻璃圆形、步骤类型图标行、统计徽章和FloatingPanel按钮玻璃化、个人中心箭头玻璃圆形；WPF 端 TabItem 升级为玻璃胶囊选项卡（ColorAnimation 过渡）。涉及文件：MainActivity.java（+import LinearGradient/Shader、+glassCapsuleDrawable）、FloatingTriggerButton.java、MainWindow.xaml、CHANGELOG/WORKLOG/LIQUID_GLASS_DESIGN/VERSION。NavButtonStyle IsMouseOver Trigger 改为 EnterActions/ExitActions ColorAnimation（150ms）；PrimaryButtonStyle 增加光泽扫过 ControlTemplate（hove时 ShineBorder 0.6s 循环 + RootBorder 暗化 0.85×）；ChoiceButtonStyle 增加 RenderTransform + hover 上浮 4px 动画；StepListBox ItemContainerStyle 增加 IsSelected EnterActions/ExitActions 背景色过渡；FloatingControlWindow 展开面板增加 ScaleY 弹性展开 + 淡入 EventTrigger；MainWindow.xaml.cs 新增 SetupPageTransitionAnimations/HandlePageVisibilityChanged/AnimateNavButtonActive 三个方法，实现页面切换淡入+上移 + 导航按钮颜色同步过渡；App.xaml 增补 PrimaryColor/PrimaryLightColor/TextSecondaryColor 纯色资源（浅深各 3 个）；新增 4 个共享 EasingFunction 资源。涉及文件：MainWindow.xaml、MainWindow.xaml.cs、FloatingControlWindow.xaml、App.xaml、CHANGELOG/WORKLOG/LIQUID_GLASS_DESIGN/VERSION。
- 2026-07-12 04:00 Phase C 移动端动效系统：MainActivity.java 新增 animatePopIn/animateSlideUp/animatePressFeedback/animateStaggeredPopIn/animateDialogEntrance 5 个辅助方法；showPage 末尾加 animateSlideUp；buildHomePage 末尾加 2×2 网格错位弹入；buildGridCard 加 OnTouchListener 按压反馈；recentCard/textCol/execBtn 加 animatePressFeedback；refreshLibrary/renderSteps/renderLog 末尾加错位弹入；showNewTaskDialog/showStepEditorDialog/showExecutionConfirmDialog 加 animateDialogEntrance；FloatingTriggerButton.java expand/collapse 加 TranslationX 滑入动画 + DecelerateInterpolator；updateBottomNav 加选中项轻弹反馈；onResume 取点恢复后加 stepsContainer 错位弹入。涉及文件：MainActivity.java、FloatingTriggerButton.java、CHANGELOG/WORKLOG/LIQUID_GLASS_DESIGN/VERSION。Android 端 glassDrawable 改为 TOP_BOTTOM 三停渐变（含 bleed）、pageBackgroundDrawable 暖冷光改用 getThemeColor() 动态获取、colors.xml/colors-night.xml 各增加 3 个颜色资源；WPF 端 App.xaml 增加 GlassBleedBrush、CardStyle 第④层改为品牌蓝光晕泄漏。涉及文件：MainActivity.java、colors.xml×2、MainWindow.xaml、App.xaml、CHANGELOG/WORKLOG/LIQUID_GLASS_DESIGN。
- 2026-07-12 03:00 Phase A 液态玻璃 Specular 高光 + 三阶阴影系统升级：Android 端 glassDrawable 改为 LayerDrawable 增加高光层、pageBackgroundDrawable 增加双径向光晕、新增三阶阴影常量和 createCard 重载；WPF 端 MainWindow.xaml 增加三阶阴影资源和 4 层 CardStyle ControlTemplate、导航栏叠加品牌蓝氛围光；FloatingControlWindow 阴影升级为 ShadowLg；更新 CHANGELOG/WORKLOG/LIQUID_GLASS_DESIGN 三份文档。涉及文件：MainActivity.java、MainWindow.xaml、FloatingControlWindow.xaml、VERSION。
